...
def makeLocation(filename, n, titles='', pdicts=[]):
    ...
    if filename.endswith('.txt'):
        ...
        if pdicts != None:
            f.write(str(n) + '\n')
            f.write('Location Index:' + ':'.join(titles) + '\n')
            for i in range(n):
                f.write(str(i))
                for t in range(k):
    ...
    if filename.endswith('.csv'):
        with open(filename, 'w', newline='') as file:
            if pdicts != None:
                fieldnames = ['Location Index'] + titles
                writer = DictWriter(file, fieldnames=fieldnames)
                writer.writeheader()
                for i in range(n):
                    tdict = {'Location Index': i}
                    for t in range(k):
                        index = random.randint(0, len(div[t]) - 1)
                ...
                fieldnames = ['Location Index']
                writer = DictWriter(file, fieldnames=fieldnames)
                writer.writeheader()
                for i in range(n):
                    writer.writerow({'Location Index': i})
...
if __name__ == '__main__':
    ...
    if args.number:
        if(args.proportion):
            ...
            makeLocation(args.filename, int(args.number))
    elif args.addLine:
