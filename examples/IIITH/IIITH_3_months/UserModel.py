...
def event_recieve_fn(agent,ambient_infection,event_info,location,current_time_step):
	...
	"""
	lockdown_coefficient = 1
	if current_time_step >19:
		lockdown_coefficient = 0.05
...
def external_prevalence(agent, current_time_step):
	...
	if(agent.info['Type'] in ['Others', 'Staff', 'Faculty']):
		if current_time_step <=19:
			return current_time_step/19*0.01

		elif current_time_step >19 and current_time_step <=35:
			return 0

		elif current_time_step >35:
			return 0
	return 0

def incubation_fn(time_step):
	return np.random.lognormal(1.6, 0.52, 1)[0]


class UserModel(Model.ScheduledModel):
	def __init__(self):
		Model.ScheduledModel.__init__(self)
		self.insert_state('Susceptible',None, None,self.p_infection([0.3,0.1],None,{'Exposed':1}),False,0.99)
		self.insert_state_custom('Exposed', incubation_fn, self.p_function({'Symptomatic':0.4,'Asymptomatic':0.6}),False,0.0)
		self.insert_state('Symptomatic',11,1,self.scheduled({'Recovered':1}),True,0.01)
