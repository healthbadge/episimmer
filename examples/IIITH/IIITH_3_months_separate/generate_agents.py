...
def write_agents_txt(filename, union_students):
	...
	header='Agent Index:Type:VolTest'
	...
	for st_roll in union_students:
		if(rand_num>0.4):
			f.write(str(st_roll)+':Student:True\n')
		else:
			f.write(str(st_roll)+':Student:False\n')

	for i in range(num_faculty):
		if(rand_num>0.4):
			f.write(str(i)+'F:Faculty:True\n')
		else:
			f.write(str(i)+'F:Faculty:False\n')

	for i in range(num_staff):
		f.write(str(i)+'S:Staff:True\n')

	for i in range(num_others):
		f.write(str(i)+'O:Others:True\n')
