...
def probabilityOfInfection_fn(p_infected_states_list,contact_agent,c_dict,current_time_step):
	if contact_agent.state=='Infected':
		return 0.001  #This is the probability of getting infected from contact in a time step isf contact is infected
...
class UserModel(model.StochasticModel):
	def __init__(self):
		...
		self.set_transition('Susceptible', 'Infected', self.p_infection(probabilityOfInfection_fn,None))	#Adding S-> I transition which is based on probability)fInfection_fn
