...
class ReadConfiguration():
	...
	def get_file_paths(self, example_path):
		...
		if self.interactions_files_list_list != ['']:
			interactions_FilesList_filename = [osp.join(
				example_path, interactions_files_list) for interactions_files_list in self.interactions_files_list_list]

		if self.events_files_list_list != ['']:
			events_FilesList_filename = [osp.join(
				example_path, events_files_list) for events_files_list in self.events_files_list_list]

		if self.locations_filename != '':
			locations_filename = osp.join(example_path, self.locations_filename)

		if self.one_time_event_file != '':
			one_time_event_file = osp.join(example_path, self.one_time_event_file)

		if self.probabilistic_interactions_files_list_list != '':
			probabilistic_interactions_FilesList_filename = [osp.join(example_path, interactions_files_list) for interactions_files_list in self.probabilistic_interactions_files_list_list]
	...
	def get_file_names_list(self, example_path, interactions_FilesList_filename, events_FilesList_filename, probabilistic_interactions_FilesList_filename):
		...
		if self.interactions_files_list_list == ['']:
			...
			interactionFiles_obj = [ReadFilesList(file) for file in interactions_FilesList_filename]
		...
		if self.probabilistic_interactions_files_list_list == ['']:
			...
			probabilistic_interactionFiles_obj = [ReadFilesList(file) for file in probabilistic_interactions_FilesList_filename]
		...
		if self.events_files_list_list == ['']:
			...
			eventFiles_obj = [ReadFilesList(file) for file in events_FilesList_filename]
