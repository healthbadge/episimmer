...
class Simulate():
	...
	@Statistics.save_stats([("agents_obj",3)], "Agents", ["can_recieve_infection"])
	def onStartTimeStep(self, interactionFiles_listOfList, eventFiles_listOfList, probabilistic_interactionFiles_listOfList, oneTimeEvent_obj):
		...
		for interactionFiles_list in interactionFiles_listOfList:
			if interactionFiles_list != []:
				interactions_filename = interactionFiles_list[Time.Time.get_current_time_step() % len(interactionFiles_list)]
		...
		for probabilistic_interactionFiles_list in probabilistic_interactionFiles_listOfList:
			if probabilistic_interactionFiles_list != []:
				probabilistic_interactions_filename = probabilistic_interactionFiles_list[Time.Time.get_current_time_step() % len(probabilistic_interactionFiles_list)]
		...
		for eventFiles_list in eventFiles_listOfList:
			if eventFiles_list != []:
				events_filename = eventFiles_list[Time.Time.get_current_time_step() % len(eventFiles_list)]
		...
		for policy in self.policy_list:
			policy.enact_policy(Time.Time.get_current_time_step(), self.agents_obj.agents.values(), self.locations_obj.locations.values(), self.model)
			...
			for location in self.locations_obj.locations.values():
				if not location.lock_down_state:
					for event_info in location.events:
						self.model.update_event_infection(event_info, location, self.agents_obj, self.event_restriction_fn)
