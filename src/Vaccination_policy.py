...
class Result():
    def __init__(self,vaccine_name,agent,result,time_step,efficacy,decay_days, current_dose):
...
class Vaccine_type():
    ...
    def vaccinate(self, agent, time_step, dose=1):
        ...
        # vaccinate agents
        result = agent.get_policy_history("Vaccination")[-1].result if dose > 1 else self.inject_agent(agent)
        if result == "Successful":
            decay_days = self.decay_days[dose - 1] if self.dosage else self.decay_days
        else:
            decay_days = 0
...
class Vaccination_policy(Agent_Policy):
    def __init__(self,agents_per_step_fn=None):
        ...
        self.vaccines=[]
        self.statistics={}
        self.statistics_total = {'Total Vaccination': [], 'Total Successful': [], 'Total Unsuccessful': []}
    ...
    def random_vaccination(self, parameter=None, value_list=[]):
        ...
        return partial(self.full_random_vaccination, parameter=parameter, value_list=value_list)

    def full_random_vaccination(self, agents, time_step, parameter, value_list):
    ...
    def multi_dose_vaccines(self, parameter=None, value_list=[]):
        return partial(self.full_multi_dose_vaccines, parameter=parameter, value_list=value_list)

    def full_multi_dose_vaccines(self, agents, time_step, parameter, value_list):
        agents_copy = copy.copy(list(agents))
        random.shuffle(agents_copy)
        curr_agents_to_vaccinate = self.num_agents_to_vaccinate
            ...
            if parameter is None or agent.info[parameter] in value_list:
                if agent.get_policy_state('Vaccination') is None and self.vaccines:
                    ...
                    curr_agents_to_vaccinate -= 1
                      ...
                      self.available_vaccines[lh.vaccine_name]["interval"][lh.current_dose - 1]
                      ):
                    ...
                    if current_vaccine is not None:
                        result = current_vaccine.vaccinate(agent, time_step, lh.current_dose + 1)
                        self.results.append(result)
                        self.vaccines.remove(current_vaccine)
                        curr_agents_to_vaccinate -= 1
        ...
        if dosage == "Single":
            ...
            for name, vaccine in vaccines.items():
                if len(vaccine["decay"]) != vaccine["dose"]:
                    raise ValueError("Vaccine decay must be a list of length equal to the count of vaccine dosage")
    ...
    def set_protection(self,agents):
        for agent in agents:
            ...
            if len(history) == 0 or history[-1].result == "Unsuccessful" or history[-1].protection == 0:
                continue
            else:
