...
class TestExamples(unittest.TestCase):
    ...
    def test_examples(self):
        ...
        for j in sub_examples_list:
            examples_list = [p for p in os.listdir(osp.join("examples",str(j))) if osp.isdir(osp.join("examples",str(j),p)) and osp.isfile(osp.join("examples",str(j),p,"config.txt"))]
    ...
    def test_vd_examples(self):
        print()
        examples_list = [p for p in os.listdir(osp.join("examples","Vulnerability_Detection")) if osp.isfile(osp.join("examples","Vulnerability_Detection", p,"vd_config.txt"))]
        for i,example in enumerate(examples_list):
            main_path = osp.join("src","Main.py")
            example_path = osp.join("examples","Vulnerability_Detection",example)
